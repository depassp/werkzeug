    ...
    try:
        ...
        def _rename(src, dst):
            if not isinstance(src, text_type):
                src = text_type(src, sys.getfilesystemencoding())
            if not isinstance(dst, text_type):
                dst = text_type(dst, sys.getfilesystemencoding())
    ...
    def rename(src, dst):
        ...
        try:
            ...
            if e.errno != errno.EEXIST:
                raise
            old = "%s-%08x" % (dst, random.randint(0, MAXSIZE))
            os.rename(dst, old)
            os.rename(src, dst)
            try:
                os.unlink(old)
